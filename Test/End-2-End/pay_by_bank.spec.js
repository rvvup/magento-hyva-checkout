...
async function placeOrderPayByBank(page, { reloadBeforePayment = false } = {}) {
  ...
  // Prevent clicking 'Place Order' too fast
...
test("Can place an order using pay by bank", async ({ page }) => {
  await placeOrderPayByBank(page);
});

test("Can place an order using pay by bank after reloading checkout", async ({
  page,
}) => {
  await placeOrderPayByBank(page, { reloadBeforePayment: true });
});
