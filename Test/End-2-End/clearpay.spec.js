...
test.skip('Can place an order', async ({ page, browser }) => {
    ...
    await frame.getByLabel('Email or mobile number').fill('playwright-' + randomString + '@example.com');
    await frame.getByRole('button', { name: 'Continue'}).click();

    await frame.getByLabel('Mobile number').fill('+777777' + Math.floor(Math.random() * 100000));
    ...
    await frame.getByTestId('verify-code-entry-input').fill('111111', {force: true});

    await frame.getByTestId('payment-method-cardNumber-input').fill('4111 1111 1111 1111');
